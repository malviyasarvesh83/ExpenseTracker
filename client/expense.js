...
const showExpense = async (response) => {
  document.getElementById(
    "loginName"
  ).textContent = `Welcome ${response.data.name}`;
  try {
    let html = "";
    let totalAmount = 0;
    for (let i = 0; i < response.data.response.length; i++) {
      ...
      html +=
        '<td><button class="btn btn-warning" data-id='+ response.data.response[i]._id +' onclick="editExpense(this)">Edit</button><button class="btn btn-danger ml-3" data-id='+ response.data.response[i]._id +' onclick="deleteExpense(this)">Delete</button></td>';
      html += "</tr>";
    }
    const token = localStorage.getItem("token");
    let response1 = await axios.get(
      "http://localhost:8000/user/totalexpenses",
...
const getExpense = async (page) => {
  try {
    ...
    for (let i = 1; i <= response.data.lastPage; i++) {
      document.getElementById("pagination").innerHTML += `
        <button class="btn pageNumber active" onclick="pageNumber(${i})">${i}</button>
      `;
    }
    document.getElementById("pagination").innerHTML += `
...
const editExpense = async (ele) => {
  ...
  try {
    ...
    let response = await axios.get(`http://localhost:8000/expense/${id}`, {
      headers: { Authorization: token },
    });
...
const deleteExpense = async (ele) => {
  ...
  const token = localStorage.getItem("token");
  try {
    let response = await axios.delete(`http://localhost:8000/expense/${id}`, {
      headers: { Authorization: token },
    });
    getExpense();
  } catch (error) {
  ...
  var options = {
    ...
    handler: async (response) => {
      ...
      if (response1.data.isPremiumUser == true) {
        ...
        document.getElementById("pUser").textContent = "You are a Premium User";
        document.getElementById("leaderBoard").style.display = "inline-block";
        document.getElementById("downloadExpense").style.display =
          "inline-block";
        document.getElementById("leaderHeading").textContent = "Leader Board";
        document.getElementById("downloadFiles").textContent =
          "Downloaded Files";
      }
    },
  };
...
const next = async () => {
  try {
    ...
    getExpense(localStorage.getItem("page"));
  } catch (error) {
    console.log(error);
  }
};
